...
function MembersManagement() {
  ...
  const approveMember = async (userId, phone, fullName) => {
    ...
    if (error) {
      ...
      alert(`‚úÖ UMUNYAMURYANGO YEMEJWE!\n\nIzina: ${fullName || phone}\nTelefoni: +250 ${phone}\n\nÌ¥ë KODE: ${approvalCode}`)
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      <div className="bg-yellow-50 rounded-xl p-4">
        <h3 className="font-bold text-xl mb-4 text-yellow-700">‚è≥ Bategereje ({pendingMembers.length})</h3>
            ...
            <div key={m.id} className="bg-white rounded-lg p-3 mb-2 shadow-sm">
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <button onClick={() => approveMember(m.id, m.phone, m.full_name)} className="bg-green-600 text-white px-3 py-1 rounded-lg text-sm">Emeza</button>
                  <button onClick={() => deleteMember(m.id, m.phone, m.full_name)} className="bg-red-600 text-white px-3 py-1 rounded-lg text-sm">Siba</button>
                </div>
              </div>
            </div>
          ))
